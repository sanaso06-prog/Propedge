    ...
    <div className="section" style={{ borderColor: `${dc}33`, background: `${dc}05` }}>
      ...
      <div style={{ padding: "0.55rem 0.7rem", background: "rgba(0,0,0,0.25)", borderRadius: "7px", borderLeft: `3px solid ${dc}` }}>
        <div style={{ fontSize: "0.74rem", color: "#ccc", lineHeight: 1.65, fontFamily: "Georgia,serif" }}>
          ...
          {" "}They allow {r.opp.threeDef}% from three — {r.opp.threeDef < 34 ? "tight perimeter D" : r.opp.threeDef > 36 ? "leaving arc open" : "average closeouts"}.
...
function PropAnalyzer() {
  ...
  const analyze = () => {
    ...
    setError(""); setExpandedCat(null);
    setResult(runAnalysis(found, propType, parseFloat(propLine)));
  };
  ...
  return (
    <div>
      <div className="section">
        ...
        <button onClick={analyze} style={{ width: "100%", marginTop: "1rem", padding: "0.9rem", background: "linear-gradient(135deg,#1818cc,#4444ff)", color: "#fff", border: "none", borderRadius: "9px", fontSize: "0.8rem", fontFamily: "monospace", letterSpacing: "0.25em", textTransform: "uppercase", cursor: "pointer", boxShadow: "0 4px 28px #4444ff33" }}>
          ⚡ Analyze Prop
        </button>
      </div>
      ...
      {result && (
        <div className="fade-up">
          ...
          <div className="section">
            <div style={{ fontSize: "0.57rem", color: "#444", letterSpacing: "0.2em", fontFamily: "monospace", textTransform: "uppercase", marginBottom: "0.6rem" }}>Statistical Context</div>
    ...
    <div style={{ minHeight: "100vh", background: "#060610", color: "#e0e0f0", fontFamily: "Georgia,serif", backgroundImage: "radial-gradient(ellipse at 15% 40%,#08082a 0%,transparent 55%),radial-gradient(ellipse at 85% 10%,#0a1a08 0%,transparent 55%)" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input:focus, select:focus { outline: none; border-color: #4444ff !important; }
        @keyframes fadeUp { from{opacity:0;transform:translateY(14px)} to{opacity:1;transform:translateY(0)} }
        .fade-up { animation: fadeUp 0.5s ease forwards; }
        .section { background: rgba(255,255,255,0.025); border: 1px solid rgba(255,255,255,0.055); border-radius: 14px; padding: 1.2rem; margin-bottom: 0.9rem; }
      `}</style>
